...
import com.example.coffeecraft.Helper.TinyDB
...
class ManagmentCart(val context: Context) {
    ...
    fun insertItems(item: ItemsModel) {
        ...
        if (existAlready) {
            ...
            listItem[index].sizePrice = item.sizePrice
        } else {
            listItem.add(item)
        }
    ...
    fun getTotalFee(): Double {
        ...
        for (item in listItem) {
            fee += (item.price + item.sizePrice) * item.numberInCart
        }
        return fee
    }

    fun updateSizePrice(itemTitle: String, newSizePrice: Int) {
        val listItem = getListCart()
        val index = listItem.indexOfFirst { it.title == itemTitle }

        if (index != -1) {
            listItem[index].sizePrice = newSizePrice
            tinyDB.putListObject("CartList", listItem)
        }
    }
