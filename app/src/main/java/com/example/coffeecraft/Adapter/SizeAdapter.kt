...
import com.example.coffeecraft.databinding.ViewholderSizeBinding

class SizeAdapter(val items: MutableList<String>) : RecyclerView.Adapter<SizeAdapter.Viewholder>() {

    private var selectedPosition = -1
    private lateinit var context: Context
    private var sizeClickListener: ((String) -> Unit)? = null
    ...
    override fun onBindViewHolder(holder: Viewholder, position: Int) {
        holder.binding.root.setOnClickListener {
            ...
            sizeClickListener?.invoke(items[position])
    ...
    fun setOnSizeClickListener(listener: (String) -> Unit) {
